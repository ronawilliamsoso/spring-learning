...
public class CrawlExportJob
    ...
    @Autowired( required = false )
    public void setImportCrawlFeedJob( @Qualifier( "exportCrawlCsvFileJob" ) final Job importCrawlCsvFileJob ) {
    ...
    protected void executeInternal( @NonNull final JobExecutionContext context )
        ...
        try {
            if ( importCrawlFeedJob != null ) {
                ...
                CrawlExportJob.LOGGER.warn( "Job execution for {}.{} skipped due missing job bean.", key.getGroup(), key.getName() );
            }
        }
