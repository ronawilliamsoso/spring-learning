...
VERSION_LABEL = os.getenv('DEPLOYMENT_GROUP_NAME')# + '-' + strftime("%Y%m%d%H%M%S")
BUCKET_KEY = os.getenv('APPLICATION_NAME') + '/' + VERSION_LABEL + '-bitbucket_builds.zip'
...
def main():
    if not upload_to_s3('target/application.zip'):
        sys.exit(1)
